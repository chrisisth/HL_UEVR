...
functions.log_info("extended_cvars.lua initializing")

local config_filename = "extended_cvars_2.txt"

local function get_mod_enabled_from_config()
	local read_val = 1 -- default to disabled
    local config_data = fs.read(config_filename)
    if config_data ~= nil  and config_data ~= "" then
        read_val = 0 -- if config file exists, mod disabled is 0
        for key, value in config_data:gmatch("([^=]+)=([^\n]+)\n?") do
            local num_val = tonumber(value)
			if key == "disableMod" then read_val = num_val end -- if config exists and disabled key exists, then value is that.
		end
	end
	return read_val
end

local disableMod = get_mod_enabled_from_config()
functions.log_info(string.format("extended_cvars: mod enabled = %d", disableMod))
...
local function check_lumen_state()
    if disableMod == 1 then return 0 end
	local result
...
-- Original CVARs
local r_Fog = 0; local r_Fog_s = false
local r_VolumetricFog = 0; local r_VolumetricFog_s = false
local r_Atmosphere = 0; local r_Atmosphere_s = false 
local r_FilmGrain = 0; local r_FilmGrain_s = false 
local r_Materials = 0; local r_Materials_s = false 

local Lumen = check_lumen_state(); local Lumen_s = false
local Nanite = check_nanite_state(); local Nanite_s = false
local Global_Illum = 0; local Global_Illum_s = false

local r_PropagateAlpha = 0; local r_PropagateAlpha_s = false
local r_MeshBlend = 0; local r_MeshBlend_s = false

-- Scalability Groups
local sg_ViewDistanceQuality = 0; local sg_ViewDistanceQuality_s = false
local sg_ShadowQuality = 0; local sg_ShadowQuality_s = false
local sg_PostProcessQuality = 0; local sg_PostProcessQuality_s = false
local sg_EffectsQuality = 0; local sg_EffectsQuality_s = false
local sg_TextureQuality = 0; local sg_TextureQuality_s = false
local sg_FoliageQuality = 0; local sg_FoliageQuality_s = false

-- LOD Settings
local r_ViewDistanceScale = 0.0; local r_ViewDistanceScale_s = false
local r_StaticMeshLODDistanceScale = 0.0; local r_StaticMeshLODDistanceScale_s = false
local r_SkeletalMeshLODDistanceScale = 0; local r_SkeletalMeshLODDistanceScale_s = false
local r_SkeletalMeshLODBias = 0; local r_SkeletalMeshLODBias_s = false
local r_StaticMeshLODBias = 0; local r_StaticMeshLODBias_s = false
local r_MipMapLODBias = 0; local r_MipMapLODBias_s = false
local r_ParticleLODBias = 0; local r_ParticleLODBias_s = false
local foliage_LODDistanceScale = 0.0; local foliage_LODDistanceScale_s = false

-- Shadow Settings
local r_ShadowQuality = 0; local r_ShadowQuality_s = false
local r_Shadow_MaxResolution = 0; local r_Shadow_MaxResolution_s = false
local r_Shadow_DistanceScale = 0.0; local r_Shadow_DistanceScale_s = false

-- Lumen Settings
local r_Lumen = 0; local r_Lumen_s = false	
local r_Lumen_Reflections_Temporal = 0; local r_Lumen_Reflections_Temporal_s = false
local r_Lumen_VSM = 0; local r_Lumen_VSM_s = false
local r_Lumen_Reflections_Allow = 0; local r_Lumen_Reflections_Allow_s = false
local r_Lumen_DiffuseIndirect_Allow = 0; local r_Lumen_DiffuseIndirect_Allow_s = false
local r_Lumen_screenProbeGather_Temporal_MaxFramesAccumulated = 0; local r_Lumen_screenProbeGather_Temporal_MaxFramesAccumulated_s = false

local function init_cvars()
    if disableMod == 1 then return end
    
    -- Original CVARs
    r_Fog = get_cvar_int("r.Fog");
    r_VolumetricFog = get_cvar_int("r.VolumetricFog")
    r_Atmosphere = get_cvar_int("r.Atmosphere")
    r_FilmGrain = get_cvar_int("r.FilmGrain")
    r_Materials = get_cvar_int("r.PostProcessing.DisableMaterials") 

    Lumen = check_lumen_state()
    Nanite = check_nanite_state()
    Global_Illum = get_cvar_int("r.DynamicGlobalIlluminationMethod")

    r_PropagateAlpha = get_cvar_int("r.PostProcessing.PropagateAlpha")
    r_MeshBlend = get_cvar_int("r.MeshBlend.Enable")

    -- Scalability Groups
    sg_ViewDistanceQuality = get_cvar_int("sg.ViewDistanceQuality")
    sg_ShadowQuality = get_cvar_int("sg.ShadowQuality")
    sg_PostProcessQuality = get_cvar_int("sg.PostProcessQuality")
    sg_EffectsQuality = get_cvar_int("sg.EffectsQuality")
    sg_TextureQuality = get_cvar_int("sg.TextureQuality")
    sg_FoliageQuality = get_cvar_int("sg.FoliageQuality")

    -- LOD Settings
    r_ViewDistanceScale = get_cvar_float("r.ViewDistanceScale")
    r_StaticMeshLODDistanceScale = get_cvar_float("r.StaticMeshLODDistanceScale")
    r_SkeletalMeshLODDistanceScale = get_cvar_float("r.SkeletalMeshLODDistanceScale")
    r_SkeletalMeshLODBias = get_cvar_int("r.SkeletalMeshLODBias")
    r_StaticMeshLODBias = get_cvar_int("r.StaticMeshLODBias")
    r_MipMapLODBias = get_cvar_int("r.MipMapLODBias")
    r_ParticleLODBias = get_cvar_int("r.ParticleLODBias")
    foliage_LODDistanceScale = get_cvar_float("foliage.LODDistanceScale")

    -- Shadow Settings
    r_ShadowQuality = get_cvar_int("r.ShadowQuality")
    r_Shadow_MaxResolution = get_cvar_int("r.Shadow.MaxResolution")
    r_Shadow_DistanceScale = get_cvar_float("r.Shadow.DistanceScale")

    -- Lumen Settings
    r_Lumen = get_cvar_int("r.Lumen")
    r_Lumen_Reflections_Temporal = get_cvar_int("r.Lumen.Reflections.Temporal")
    r_Lumen_VSM = get_cvar_int("r.Lumen.VSM")
    r_Lumen_Reflections_Allow = get_cvar_int("r.Lumen.Reflections.Allow")
    r_Lumen_DiffuseIndirect_Allow = get_cvar_int("r.Lumen.DiffuseIndirect.Allow")
    r_Lumen_screenProbeGather_Temporal_MaxFramesAccumulated = get_cvar_int("r.Lumen.screenProbeGather.Temporal.MaxFramesAccumulated")
end
...
local function write_config()
	...
	if disableMod == 1 then config = config .. string.format("disableMod=%d\n", disableMod) end
	...
	-- Lumen settings
    if r_Lumen_s then config = config .. string.format("r_Lumen=%d\n", r_Lumen) end
    if r_Lumen_VSM_s then config = config .. string.format("r_Lumen_VSM=%d\n", r_Lumen_VSM) end
    if r_Lumen_Reflections_Temporal_s then config = config .. string.format("r_Lumen_Reflections_Temporal=%d\n", r_Lumen_Reflections_Temporal) end
    if r_Lumen_Reflections_Allow_s then config = config .. string.format("r_Lumen_Reflections_Allow=%d\n", r_Lumen_Reflections_Allow) end
    if r_Lumen_DiffuseIndirect_Allow_s then config = config .. string.format("r_Lumen_DiffuseIndirect_Allow=%d\n", r_Lumen_DiffuseIndirect_Allow) end
    if r_Lumen_screenProbeGather_Temporal_MaxFramesAccumulated_s then config = config .. string.format("r_Lumen_screenProbeGather_Temporal_MaxFramesAccumulated=%d\n", r_Lumen_screenProbeGather_Temporal_MaxFramesAccumulated) end
...
local function read_config()
    ...
    if config_data then
        for key, value in config_data:gmatch("([^=]+)=([^\n]+)\n?") do
            ...
            elseif key == "r_Lumen" then r_Lumen = num_val; r_Lumen_s = true
            elseif key == "r_Lumen_VSM" then r_Lumen_VSM = num_val; r_Lumen_VSM_s = true
            elseif key == "r_Lumen_Reflections_Temporal" then r_Lumen_Reflections_Temporal = num_val; r_Lumen_Reflections_Temporal_s = true
            elseif key == "r_Lumen_Reflections_Allow" then r_Lumen_Reflections_Allow = num_val; r_Lumen_Reflections_Allow_s = true
            elseif key == "r_Lumen_DiffuseIndirect_Allow" then r_Lumen_DiffuseIndirect_Allow = num_val; r_Lumen_DiffuseIndirect_Allow_s = true
            elseif key == "r_Lumen_screenProbeGather_Temporal_MaxFramesAccumulated" then r_Lumen_screenProbeGather_Temporal_MaxFramesAccumulated = num_val; r_Lumen_screenProbeGather_Temporal_MaxFramesAccumulated_s = true
			elseif key == "disableMod" then disableMod = num_val;
            end
        end
    end
end
...
local function apply_cvars()
    ...
    -- Lumen Settings
    set_cvar_int("r.Lumen", r_Lumen)
    set_cvar_int("r.Lumen.VSM", r_Lumen_VSM)
...
uevr.lua.add_script_panel("Extended CVARs", function()
	...
	if disableMod == 0 then
		...
		if imgui.collapsing_header("Lumen / Nanite Settings") then
			val, readable = get_cvar_int("r.Lumen")
			if readable == true then
				local changed, new_value = imgui.checkbox("r.Lumen", r_Lumen == 1)
				if changed then r_Lumen = new_value and 1 or 0; needs_config_write = true; r_Lumen_s = true end
			end

			val, readable = get_cvar_int("r.Lumen.VSM")
			if readable == true then
				local changed, new_value = imgui.checkbox("r.Lumen.VSM", r_Lumen_VSM == 1)
				if changed then r_Lumen_VSM = new_value and 1 or 0; needs_config_write = true; r_Lumen_VSM_s = true end
			end

			val, readable = get_cvar_int("r.Lumen.Reflections.Allow")
			if readable == true then
				local changed, new_value = imgui.checkbox("r.Lumen.Reflections.Allow", r_Lumen_Reflections_Allow == 1)
				if changed then r_Lumen_Reflections_Allow = new_value and 1 or 0; needs_config_write = true; r_Lumen_Reflections_Allow_s = true end
			end
			...
			val, readable = get_cvar_int("r.Lumen.DiffuseIndirect.Allow")
			if readable == true then
				local changed, new_value = imgui.checkbox("r.Lumen.DiffuseIndirect.Allow", r_Lumen_DiffuseIndirect_Allow == 1)
				if changed then r_Lumen_DiffuseIndirect_Allow = new_value and 1 or 0; needs_config_write = true; r_Lumen_DiffuseIndirect_Allow_s = true end
			end

			val, readable = get_cvar_int("r.Lumen.screenProbeGather.Temporal.MaxFramesAccumulated")
			if readable == true then
				local changed, new_value = imgui.slider_int("r.Lumen.screenProbeGather.Temporal.MaxFramesAccumulated", r_Lumen_screenProbeGather_Temporal_MaxFramesAccumulated, 0, 64)
				if changed then r_Lumen_screenProbeGather_Temporal_MaxFramesAccumulated = new_value; needs_config_write = true; r_Lumen_screenProbeGather_Temporal_MaxFramesAccumulated_s = true end
			end
	...
	local changed, new_value = imgui.checkbox("Disable Mod For This Game", disableMod == 1)
	if changed then 
        disableMod = new_value and 1 or 0; 
        needs_config_write = true; 
        if disableMod == 0 then
            functions.log_info("mod is being enabled. Initializing cvars")
            init_cvars()
            functions.log_info("Cvars are now initialized")
        end
    end
...
if disableMod == 0 then
    functions.log_info("Initializing cvars")
    init_cvars()
    functions.log_info("Reading config")
	read_config()
    functions.log_info("Applying Cvars")
	apply_cvars()
end